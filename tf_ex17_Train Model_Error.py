...
# 假如需要保存y_prob，以便在预测时使用
tf.add_to_collection('pred_network', y_prob)
...
# RMSProp 收斂最快!
# solver = tf.train.RMSPropOptimizer(0.3).minimize(cross_entropy)
...
for step in range(5000):
	...
	if isTrain:
		...
		_, train_loss = sess.run([solver, cross_entropy], feed_dict={xs: img_batch, ys: label_batch }  )
		# print("Training step: " + str(step) + " " + str(train_loss))
		# print(train_label[0])
		# print(label_batch[0])
		# print()
	...
	if step%50 == 0:
		...
		test_label, test_loss = sess.run([y_prob, cross_entropy], feed_dict={xs: img_batch, ys: label_batch})
		print("Testing step: " + str(step) + " " + str(test_loss))
		print(test_label[0])
		print(label_batch[0])
		print()
		# 保存checkpoint, 同时也默认导出一个meta_graph
		# graph名为'my-model-{global_step}.meta'.
		save_path = saver.save(sess, "save/" + saveName, global_step=5000)
