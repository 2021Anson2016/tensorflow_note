...
# define placeholder for inputs to network
# xs = tf.placeholder(tf.float32, [batch_size, 64, 64], name='x_input')  # 64X64，xs 是所有圖片例子
xs = tf.placeholder(tf.float32, [64, 64], name='x_input')  # 64X64，xs 是所有圖片例子； 1: channel

# ys = tf.placeholder(tf.float32, [batch_size, 3], name='y_input')  # 3:label size
# ys = tf.placeholder(tf.float32, [1, 3], name='y_input')  # 3:label size
ys = tf.placeholder(tf.float32, [1, 3], name='y_input')  # 3:label size

# keep_prob = tf.placeholder(tf.float32)
# xs_re = tf.reshape(xs, [batch_size, 64, 64, 1])
xs_re = tf.reshape(xs, [1, 64, 64, 1])
...
while 1:
    ...
    # for it in range(1):
    # if it % 50 == 0:  # testing
    # img_batch, label_batch = next_batch(img_re, labels_test, batch_size)
    # y__, loss_ = sess.run([y_prob, cross_entropy], feed_dict={xs: img_batch, ys: label_batch})

    y__, loss_ = sess.run([y_prob, cross_entropy], feed_dict={xs: img_reGray, ys: labels_test})
